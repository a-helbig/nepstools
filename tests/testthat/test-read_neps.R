dp_semantic_neps_gap <- test_path("test_data", "SC6_spGap_S_15-0-0.dta")
dp_non_neps_example <- test_path("test_data", "example-non-neps-dataset.dta")


test_that("is the generated object of type dataframe?", {
  expect_true(is.data.frame(read_neps(dp_semantic_neps_gap)))
})
...
test_that("does the english argument work?", {
  ...
  expect_equal(attr(read_neps(dp_semantic_neps_gap, english =F)$ID_t, "label"), "Target-ID")
})

test_that("does the compact_meta argument work?", {
  expect_equal(length(attributes(read_neps(dp_semantic_neps_gap, compact_meta = F)$ID_t)), 8)
  expect_equal(length(attributes(read_neps(dp_semantic_neps_gap, compact_meta = T)$ID_t)), 3)
})

test_that("does the charren argument work?", {
  expect_equal(names(read_neps(dp_semantic_neps_gap, charren = F)[13]), "ts29901")
  expect_equal(names(read_neps(dp_semantic_neps_gap, charren = T)[13]), "h_aktlue")
})
...
test_that("error if col_select contains variables not present in data", {
  expect_error(read_neps(dp_semantic_neps_gap, col_select = c("nonexistent_var")))
})
...
test_that("meta types filtered correctly based on language", {
  ...
  meta_de <- unique(read_exp_fields(dp_semantic_neps_gap)$type)
  meta_de2 <- meta_de[!stringr::str_detect(meta_de, "_en$")]

  # Extract meta_data for english = TRUE
  meta_en <- unique(read_exp_fields(dp_semantic_neps_gap)$type)
...
test_that("read_exp_fields returns a dataframe with expected columns", {
  df_exp_fields <- read_exp_fields(dp_semantic_neps_gap)
